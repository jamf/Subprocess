...
final class ShellTests: XCTestCase {
    ...
    func testExecWithFileInput() {
        ...
        // When
        XCTAssertNoThrow(result = try Shell(["/usr/bin/grep", "ProductVersion"]).exec(input: .file(path: softwareVersionFilePath), encoding: .utf8))
    ...
    func testExecReturningBoolFromString() {
        ...
        // When
        XCTAssertNoThrow(result = try Shell(["/usr/bin/dscl", ".", "list", "/Users"]).exec(encoding: .utf8, transformBlock: { _, txt in
    ...
    func testExecReturningPropertyList() {
        ...
        // Then
        guard let versionNumber = result?["ProductVersion"] as? String else { return XCTFail() }
    ...
    func testExecReturningDecodableObjectFromJSON() {
        ...
        // When
        XCTAssertNoThrow(result = try Shell(["/usr/bin/log", "show", "--style", "json", "--last", "5m"]).exec(decoder: JSONDecoder()))
    ...
    func testExecReturningDecodableObjectFromPropertyList() {
        ...
        // Then
        guard let versionNumber = result?.version else { return XCTFail() }
        XCTAssertTrue(fullVersionString.contains(versionNumber))
    }
}
